...
internal class DefaultPrepareForPreviewUseCase(
    ...
    override fun invoke(text: String, mode: MarkupMode): String = when (mode) {
        MarkupMode.BBCode -> bbCodeConverter.toHtml(text)
        MarkupMode.Markdown -> markdownConverter.toHtml(text)
        else -> text
    }.withMentions().withHashtags()

    private fun String.withMentions(): String = ContentRegexes.USER_MENTION.substituteAllOccurrences(this) { match ->
    ...
    private fun String.withHashtags(): String = ContentRegexes.HASHTAG.substituteAllOccurrences(this) { match ->
