...
class DefaultDeleteAccountUseCaseTest {
    ...
    @Test
    fun `given account is not the active one when executed then interactions are as expected`() = runTest {
        val accountId = 1L
    ...
    fun `given account is active when executed then interactions are as expected`() = runTest {
        val accountId = 1L
        val account = AccountModel(id = accountId)
