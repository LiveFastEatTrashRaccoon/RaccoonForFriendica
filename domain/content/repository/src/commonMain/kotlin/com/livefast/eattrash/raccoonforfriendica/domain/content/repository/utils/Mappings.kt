...
internal fun StatusSource.toModel() = TimelineEntryModel(
    content = text.orEmpty(),
    id = id,
    spoiler = spoilerText,
)

internal fun PreviewCard.toModel() = PreviewCardModel(
    title = title,
...
internal fun StatusContext.toModel() = TimelineContextModel(
    ancestors = ancestors.map { it.toModelWithReply() },
    descendants = descendants.map { it.toModelWithReply() },
)

internal fun MediaAttachment.toModel() = AttachmentModel(
    blurHash = blurHash,
    description = description,
    id = id,
...
internal fun MediaTypeDto.toModel(): MediaType = when (this) {
...
internal fun Visibility.toDto(): String = when (this) {
...
internal fun Account.toModel() = UserModel(
    avatar = avatar,
    bio = note,
    bot = bot,
    created = createdAt,
...
internal fun CredentialAccount.toModel() = UserModel(
    avatar = avatar,
...
internal fun Field.toModel() = FieldModel(
    key = name,
    value = value,
    verified = verifiedAt != null,
)

internal fun HistoryItem.toModel() = HashtagHistoryItem(
    day = day,
    users = accounts,
    uses = uses,
)

internal fun Tag.toModel() = TagModel(
...
internal fun Relationship.toModel() = RelationshipModel(
    blocking = blocking,
...
internal fun TrendsLink.toModel() = LinkModel(
...
internal fun FriendicaPhoto.toModel() = AttachmentModel(
    album = album,
    description = desc,
    id = id,
...
internal fun UserListReplyPolicy.toModel(): CircleReplyPolicy = when (this) {
...
internal fun CircleReplyPolicy.toDto(): String = when (this) {
    CircleReplyPolicy.Followed -> "followed"
    CircleReplyPolicy.List -> "list"
    CircleReplyPolicy.None -> "none"
}

internal fun UserList.toModel() = CircleModel(
    exclusive = exclusive,
    id = id,
    name = title,
...
private fun String.toCircleType(): CircleType = when {
    startsWith("channel:") -> CircleType.Predefined
    startsWith("group:") -> CircleType.Group
    else -> CircleType.UserDefined
}

internal fun FriendicaCircle.toModel() = CircleModel(
    id = id,
    name = title,
)

internal fun Poll.toModel() = PollModel(
    expired = expired,
    expiresAt = expiresAt,
    id = id,
    multiple = multiple,
...
internal fun PollOption.toModel() = PollOptionModel(
    title = title,
    votes = votesCount,
)

internal fun Instance.toModel() = NodeInfoModel(
...
internal fun InstanceRule.toModel() = RuleModel(
    id = id,
    text = text,
)

internal fun FriendicaContact.toModel() = UserModel(
...
internal fun FriendicaPrivateMessage.toModel() = DirectMessageModel(
    created =
    createdAt?.let { date ->
        parseDate(
            value = date,
...
internal fun ScheduledStatus.toModel() = TimelineEntryModel(
    attachments = attachments.map { it.toModel() },
...
internal fun ReportCategory.toDto(): String = when (this) {
...
internal fun CustomEmoji.toModel() = EmojiModel(
    url = url,
    code = shortCode,
    staticUrl = staticUrl,
    category = category,
...
internal fun Markers.toModel(): List<MarkerModel> = buildList {
...
internal fun Event.toModel() = EventModel(
    id = id.toString(),
    uri = uri,
    title = name,
    description = description,
    ...
    endTime?.let { date ->
        parseDate(
            ...
            val year = t.extractDatePart().year
            year > 1970
        }
    },
...
internal fun StatusReaction.toModel() = ReactionModel(
    count = count,
    isMe = me == true,
    name = name,
    staticUrl = staticUrl,
...
internal fun Announcement.toModel() = AnnouncementModel(
    id = id,
    content = content,
